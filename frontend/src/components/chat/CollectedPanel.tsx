  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        <h3 style={{ fontSize: 14, fontWeight: 700, margin: 0, color: '#24292f' }}>
      ...
      {passages.length > 0 && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
          <button
            ...
            onClick={copyAll}
            style={{
              background: copied ? '#E8F5E9' : '#F1F8FF',
              border: '1px solid', borderColor: copied ? '#A5D6A7' : '#BBD9F5',
              borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontSize: 12,
              color: copied ? '#2E7D32' : '#0969da', fontWeight: 600,
            }}
          >
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                  <span style={{ fontSize: 11, color: '#0969da', fontWeight: 600 }}>{p.query}</span>
                  <button
                    type="button"
                    onClick={() => onRemove(p.id)}
                    title="Remove"
                    style={{ background: 'none', border: 'none', color: '#bbb', cursor: 'pointer', fontSize: 14, lineHeight: 1, padding: 0 }}
                  >
                    ×
                  </button>
                </div>
                <div style={{ fontSize: 13, lineHeight: 1.5, color: '#222', marginTop: 4, whiteSpace: 'pre-wrap' }}>
                  {p.text}
                </div>
                {attribution && (
                  <div style={{ fontSize: 11, color: '#888', marginTop: 6 }}>— {attribution}</div>
                )}
              </div>
            )
          })
