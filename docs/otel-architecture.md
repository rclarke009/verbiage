...
When tracing is on, log records get `trace_id` and `span_id` fields (printed in the log format). Each `/ask` also emits a structured JSON line (`event=ask_run`) and returns the same summary on `AskResponse.ask_run` / SSE — see **[debugging-ask-bugs.md](debugging-ask-bugs.md)** (triage playbook) and **[ask-run-diagnosis.md](ask-run-diagnosis.md)** (field reference). Shipping those lines to Loki for click-through from Tempo remains a future step.
